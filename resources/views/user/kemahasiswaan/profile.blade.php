...
@section('content')
    <div class="container px-4 py-8 mx-auto">
        ...
        <div class="mx-auto max-w-3xl bg-white rounded-xl border border-gray-100 shadow-lg">
                ...
                <div class="flex justify-center items-center space-x-6">
                    <div class="flex relative flex-col items-center">
                        ...
                        <div class="flex mt-6 space-x-3">
                            <form action="{{ route('kemahasiswaan.profile.photo.update') }}" method="POST" enctype="multipart/form-data" id="photoForm" class="inline">
                                @csrf
            ...
            <form action="{{ route('kemahasiswaan.profile.update') }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
    ...
    <script>
        ...
        function updateEmailVerificationStatus() {
            fetch('{{ route('kemahasiswaan.email.verification.status') }}')
        ...
        function setupVerificationHandlers() {
            ...
            emailInput.addEventListener('change', function() {
                ...
                if (newEmail && newEmail !== '{{ $kemahasiswaan->email }}') {
                    const formData = new FormData();
                    formData.append('email', newEmail);

                    fetch('{{ route('kemahasiswaan.email.show.verification') }}', {
                        method: 'POST',
                        headers: {
            ...
            sendOtpBtn.addEventListener('click', function() {
                ...
                fetch('{{ route('kemahasiswaan.email.send.otp') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
            ...
            verifyOtpBtn.addEventListener('click', function() {
                ...
                fetch('{{ route('kemahasiswaan.email.verify.otp') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
            ...
            resendOtpBtn.addEventListener('click', function() {
                ...
                fetch('{{ route('kemahasiswaan.email.resend.otp') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
