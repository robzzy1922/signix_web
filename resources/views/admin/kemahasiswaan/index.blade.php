...
<div class="container max-w-full px-4 py-8 mx-auto overflow-x-hidden sm:px-6 lg:px-8">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 font-medium text-gray-500 md:ml-2">Kemahasiswaan</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-6 sm:flex sm:items-center sm:justify-between">
        <h1 class="text-2xl font-semibold text-gray-900">Daftar Kemahasiswaan</h1>
        ...
        <a href="{{ route('admin.kemahasiswaan.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                    ...
                    @forelse($kemahasiswaans as $index => $kemahasiswaan)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $kemahasiswaan->nama_kemahasiswaan }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $kemahasiswaan->nip }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $kemahasiswaan->prodi }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $kemahasiswaan->email }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $kemahasiswaan->no_hp }}</div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('admin.kemahasiswaan.edit', $kemahasiswaan->id) }}"
                                    ...
                                    <form action="{{ route('admin.kemahasiswaan.destroy', $kemahasiswaan->id) }}"
                                        method="POST"
                                        class="inline-block">
                                      @csrf
                                      @method('DELETE')
                                      <button type="button"
                                              onclick="showDeleteAlert(this, '{{ $kemahasiswaan->nama_kemahasiswaan }}')"
