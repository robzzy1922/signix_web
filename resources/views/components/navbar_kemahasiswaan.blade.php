<nav x-data="{ isOpen: false }" class="flex flex-wrap justify-between items-center p-4 bg-white shadow-sm">
    ...
    <div :class="{'hidden': !isOpen}" class="w-full md:flex md:w-auto md:items-center">
        <div class="flex flex-col mt-4 md:flex-row md:space-x-6 md:mt-0">
            <a href="{{ route('kemahasiswaan.dashboard') }}" class="nav-item text-black font-medium hover:text-blue-500 py-2 md:py-0 {{ request()->routeIs('kemahasiswaan.dashboard') ? 'text-blue-500' : '' }}">
                Home
            </a>
            <a href="{{ route('kemahasiswaan.riwayat') }}" class="nav-item text-black font-medium hover:text-blue-500 py-2 md:py-0 {{ request()->routeIs('kemahasiswaan.riwayat') ? 'text-blue-500' : '' }}">
                Riwayat
            </a>
            <div class="md:hidden">
                <div class="block py-2 text-sm text-gray-700 border-b border-gray-200">
                    {{ Auth::user()->nama_kemahasiswaan }}
                </div>
                <a href="{{ route('kemahasiswaan.profile') }}" class="nav-item text-black font-medium hover:text-blue-500 py-2 {{ request()->routeIs('kemahasiswaan.profile') ? 'text-blue-500' : '' }}">
                    Profile
                </a>
                <form method="POST" action="{{ route('kemahasiswaan.logout') }}">
                    @csrf
                    <button type="submit" class="py-2 w-full font-medium text-left text-black hover:text-blue-500">Logout</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="hidden items-center space-x-4 md:flex">
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" class="text-black">
                @if(Auth::guard('kemahasiswaan')->user()->profile && file_exists(public_path('profiles/' . Auth::guard('kemahasiswaan')->user()->profile)))
            ...
            <div x-show="open" @click.away="open = false" class="absolute right-0 z-10 py-1 mt-2 w-48 bg-white rounded-md shadow-lg">
                <div class="block px-4 py-2 text-sm text-gray-700 border-b border-gray-200">
                    {{ Auth::guard('kemahasiswaan')->user()->nama_kemahasiswaan }}
                </div>
                <a href="{{ route('kemahasiswaan.profile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Profile</a>
                <form method="POST" action="{{ route('kemahasiswaan.logout') }}">
                    @csrf
